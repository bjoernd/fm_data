...
fn auth_with_google(creds_file: &str, token_file: &str) -> ConcreteAuthenticator {
    let path = std::path::Path::new(creds_file);
    let secret: ApplicationSecret = oauth2::read_application_secret(path).unwrap();
...
pub fn clear_sheet_area(hub: &GSheetsHub, sheetid: &str, sheetname: &str, sheetrange: &str) {
    ...
    let req = sheets4::BatchClearValuesRequest {
        ranges: Some(vec![request_fmt]),
    };

    let result = hub.spreadsheets().values_batch_clear(req, sheetid).doit();
...
pub fn upload_attributes(hub: &GSheetsHub, tab: &Table, sheetid: &str, sheetname: &str) {
    ...
    let result = hub
        .spreadsheets()
        .values_update(req, sheetid, request_fmt.as_str())
