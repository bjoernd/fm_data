...
pub fn format_player_data(player: &ImagePlayer) -> String {
    ...
    output.push(player.get_attribute("technical_free_kick_taking").to_string());
    ...
    output.push(player.get_attribute("goalkeeping_command_of_area").to_string());
    output.push(player.get_attribute("goalkeeping_communication").to_string());
    ...
    output.push(player.get_attribute("goalkeeping_punching_tendency").to_string());
    output.push(player.get_attribute("goalkeeping_reflexes").to_string());
    output.push(player.get_attribute("goalkeeping_rushing_out_tendency").to_string());
...
mod tests {
    ...
    fn test_format_player_data_field_player() {
        ...
        for i in 39..50 {
            assert_eq!(fields[i], "0", "Goalkeeping attribute at position {} should be 0 for field player", i);
        }
    }
    ...
    fn test_format_player_data_missing_attributes_as_zeros() {
        ...
        for i in 3..50 {
            if i != 4 { // Skip the crossing attribute we set
                assert_eq!(fields[i], "0", "Attribute at position {} should be 0", i);
            }
        }
    }
