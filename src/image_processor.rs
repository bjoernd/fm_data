...
fn locate_footedness_indicators(image: &DynamicImage) -> Result<FootednessIndicators> {
        ...
        for x in 0..(search_region.width().saturating_sub(100)) {
            ...
            if dark_pixels > 30 {
                ...
                if absolute_x < width / 2 && left_foot_region.is_none() {
                    ...
                    debug!("Located left foot indicator at ({}, {})", absolute_x, absolute_y);
                } else if absolute_x >= width / 2 && right_foot_region.is_none() {
                    right_foot_region = Some((absolute_x, absolute_y));
                    debug!("Located right foot indicator at ({}, {})", absolute_x, absolute_y);
                }
    ...
    let left_foot = left_foot_region
        .ok_or_else(|| FMDataError::image("Could not locate LEFT FOOT indicator in image".to_string()))?;
    let right_foot = right_foot_region
        .ok_or_else(|| FMDataError::image("Could not locate RIGHT FOOT indicator in image".to_string()))?;
...
fn detect_circle_colors(image: &DynamicImage, indicators: &FootednessIndicators) -> Result<Footedness> {
    debug!("Detecting circle colors for footedness determination");
    
    let rgb_image = image.to_rgb8();
    
    // Define search regions around each foot indicator  
    let left_circle_region = get_circle_search_region(&indicators.left_foot_pos, rgb_image.width(), rgb_image.height());
    let right_circle_region = get_circle_search_region(&indicators.right_foot_pos, rgb_image.width(), rgb_image.height());
    ...
    debug!("Left foot color: {:?}, Right foot color: {:?}", left_color, right_color);
    ...
    match (left_color, right_color) {
        ...
        (CircleColor::Green, CircleColor::Yellow) | (CircleColor::Green, CircleColor::Gray) => Ok(Footedness::LeftFooted),
        (CircleColor::Yellow, CircleColor::Green) | (CircleColor::Gray, CircleColor::Green) => Ok(Footedness::RightFooted),
...
fn get_circle_search_region(foot_pos: &(u32, u32), image_width: u32, image_height: u32) -> (u32, u32, u32, u32) {
    let (x, y) = *foot_pos;
...
fn detect_dominant_color_in_region(image: &RgbImage, region: &(u32, u32, u32, u32)) -> Result<CircleColor> {
    let (x, y, width, height) = *region;
    
    debug!("Analyzing color in region ({}, {}) with size {}x{}", x, y, width, height);
    ...
    debug!("Color analysis: {} green, {} yellow, {} total colored pixels", 
...
mod tests {
    ...
    fn create_test_png_with_footedness_indicators(left_color: &[u8; 3], right_color: &[u8; 3]) -> NamedTempFile {
